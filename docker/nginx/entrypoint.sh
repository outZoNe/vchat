...
if [ "$NGINX_CONFIG_FILE" = "prod.default.conf" ]; then
    ...
    if [ -f "/etc/nginx/conf.d/prod.default.conf.template" ]; then
        envsubst '${APP_DOMAIN}' < /etc/nginx/conf.d/prod.default.conf.template > /etc/nginx/conf.d/default.conf
        echo "Processed nginx config template with variables: APP_DOMAIN=${APP_DOMAIN}"
    else
